<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form action="{{ route('debates.update', $debate) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="title" id="title" value="{{ old('title', $debate->title) }}"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select name="type" id="type"
                                        ...
                                        required>
                                        <option value="friendly" {{ old('type', $debate->type->value) === 'friendly' ? 'selected' : '' }}>{{ __('Friendly') }}</option>
                                        <option value="international" {{ old('type', $debate->type->value) === 'international' ? 'selected' : '' }}>{{ __('International') }}</option>
                                        <option value="internal" {{ old('type', $debate->type->value) === 'internal' ? 'selected' : '' }}>{{ __('Internal') }}</option>
                                    </select>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="date" id="date" value="{{ old('date', $debate->date?->toDateString()) }}"
                            ...
                            <div>
                                ...
                                <input type="text" name="location" id="location" value="{{ old('location', $debate->location) }}"
                            ...
                            <div>
                                ...
                                <textarea name="outcome" id="outcome" rows="4"
                                    ...
                                    placeholder="{{ __('Optional') }}">{{ old('outcome', $debate->outcome) }}</textarea>
    ...
    <script>
        function participantForm() {
            @php
                $participantsData = old('participants');
                if ($participantsData === null) {
                    $participantsData = $debate->participants->map(fn ($p) => [
                        'person_id' => (string) $p->id,
                        'role' => $p->pivot->role,
                    ])->values()->toArray();
                }
                if (empty($participantsData)) {
                    $participantsData = [['person_id' => '', 'role' => 'debater']];
                }
            @endphp
            const oldParticipants = @json($participantsData);
            return {
