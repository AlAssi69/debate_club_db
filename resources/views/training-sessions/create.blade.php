                ...
                <div class="p-6">
                    <form action="{{ route('training-sessions.store') }}" method="POST">
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="category" name="category" type="text" class="block mt-1 w-full" :value="old('category')" />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="scheduled_date" name="scheduled_date" type="date" class="block mt-1 w-full" :value="old('scheduled_date')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="time" name="time" type="time" class="block mt-1 w-full" :value="old('time')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="duration_minutes" name="duration_minutes" type="number" min="1" class="block mt-1 w-full" :value="old('duration_minutes')" required />
                                ...
                                @forelse ($persons as $person)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="trainer_ids[]" value="{{ $person->id }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                            {{ in_array($person->id, old('trainer_ids', [])) ? 'checked' : '' }}>
                                ...
                                @forelse ($persons as $person)
                                    <label class="flex items-center">
                                        <input type="checkbox" name="trainee_ids[]" value="{{ $person->id }}" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                            {{ in_array($person->id, old('trainee_ids', [])) ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm text-gray-700">{{ $person->full_name }}</span>
                                    </label>
                                @empty
                                    <p class="text-sm text-gray-500">{{ __('No persons available.') }}</p>
